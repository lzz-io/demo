...
<mapper namespace="${mapperInterface.packageName}.${baseInfo.fileName}">
    ...
    <select id="selectByPrimaryKey"<#if (tableClass.pkFields?size==1)> parameterType="${tableClass.pkFields[0].fullTypeName}"</#if> resultMap="BaseResultMap">
        select
        <include refid="Base_Column_List" />
        from ${tableClass.tableName}
    ...
    <delete id="deleteByPrimaryKey"<#if (tableClass.pkFields?size==1)> parameterType="${tableClass.pkFields[0].fullTypeName}"</#if>>
